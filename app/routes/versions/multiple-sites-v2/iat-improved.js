// app/routes/versions/multiple-sites-v2/iat-improved.js
// Generates the full Marine-licence IAT journey using IMPROVED CONTENT.
// All paths include /versions/multiple-sites-v2/journey-improved/ prefix.

module.exports = function (router) {
  const path = require("path");
  const iat = require(path.join(__dirname, "../../../data/iat-improved-content.json"));

  const base = "/versions/multiple-sites-v2/journey-improved"; // prefix reused everywhere
  ...
  // ------- Start page for improved journey -------------------------------------------
  router.get('/versions/multiple-sites-v2/start-improved', (req, res) => {
    res.render('versions/multiple-sites-v2/start-improved');
  });
  ...
  allRouteItems.forEach((q) => {
    const fullPath = `${base}${q.route}`; // e.g. /versions/multiple-sites-v2/journey-improved/sea
    ...
    router.get(fullPath, (req, res) => {
        ...
        if (outcomeType) {
          ...
          if (outcomeType.heading) {
            ...
            if (outcomeType.heading === "Fill out an exemption notification") {
              ...
              if (article) {
                actionHref = `/versions/multiple-sites-v2/exemption/sign-in?article=${article}`;
              } else {
                actionHref = "/versions/multiple-sites-v2/exemption/sign-in";
              }
            }
          ...
          return res.render("versions/multiple-sites-v2/layouts/iat/outcome", view);
        }
      }
      ...
      if (isCard(q)) {
        ...
        res.render("versions/multiple-sites-v2/layouts/iat/card-selection", view);
      } else {
        ...
        res.render("versions/multiple-sites-v2/layouts/iat/radio-page", view);
      }
    });
  ...
  iat.outcomes
    ...
    .forEach((o) => {
      ...
      router.get(fullPath, (req, res) => {
        res.render("versions/multiple-sites-v2/layouts/iat/outcome", {
          h1: o.heading || "",
          bodyHtml: o.text,
