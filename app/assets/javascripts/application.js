...
window.GOVUKPrototypeKit.documentReady(() => {
  ...
  if (typeof accessibleAutocomplete !== 'undefined') {
    ...
    const marineOrganizations = [
      'Belfast Harbour Authority Ltd',
      'Blackpool Marina Services Ltd',
      'Brighton Marina Operations Ltd',
      'Bristol Port Company Ltd',
      'Cowes Yacht Services Ltd',
      'Dover Port Authority Ltd',
      'Exmouth Oysters Ltd',
      'Falmouth Marina Group Ltd',
      'Great Yarmouth Port Company Ltd',
      'Grimsby Fish Dock Enterprise Ltd',
      'Harwich Haven Authority Ltd',
      'Hull Marina Management Ltd',
      'Ipswich Marina Services Ltd',
      'Liverpool Marine Operations Ltd',
      'London Gateway Port Ltd',
      'Lowestoft Port Services Ltd',
      'Newcastle Port Authority Ltd',
      'Norfolk Marine Services Ltd',
      'North East Wind Farms Ltd',
      'Plymouth Port Services Ltd',
      'Poole Harbour Commissioners Ltd',
      'Portsmouth Wind Farm Ltd',
      'Ramsgate Marina Ltd',
      'Scarborough Marina Ltd',
      'Southampton Marina Ltd',
      'Tees Port Authority Ltd',
      'Whitby Marine Services Ltd'
    ]
    ...
    // ============================================================
    // Invoice country autocomplete (international invoice address)
    // Self-contained, guarded by its own container id so it cannot
    // affect the other autocompletes on the site.
    // ============================================================
    ...
    const invoiceCountryContainer = document.querySelector('#invoice-country-autocomplete-container')
    if (invoiceCountryContainer) {
      // Hidden input that actually gets submitted with the form
      const invoiceCountryHidden = document.createElement('input')
      invoiceCountryHidden.type = 'hidden'
      invoiceCountryHidden.name = 'invoice-country'
      invoiceCountryHidden.id = 'invoice-country-hidden'
      invoiceCountryContainer.parentNode.appendChild(invoiceCountryHidden)

      // Prefill (used when changing answers from the check page)
      const invoiceCountryDefault = invoiceCountryContainer.getAttribute('data-default-value') || ''
      invoiceCountryHidden.value = invoiceCountryDefault

      accessibleAutocomplete({
        element: invoiceCountryContainer,
        id: 'invoice-country',
        source: invoiceCountries,
        defaultValue: invoiceCountryDefault,
        minLength: 1,
        showAllValues: true,
        confirmOnBlur: false,
        autoselect: true,
        onConfirm: function (value) {
          invoiceCountryHidden.value = value || ''
        }
      })

      // Keep the hidden input in sync with manual typing too
      setTimeout(() => {
        const invoiceCountryInput = document.querySelector('#invoice-country')
        if (invoiceCountryInput) {
          invoiceCountryInput.addEventListener('input', function (e) {
            invoiceCountryHidden.value = e.target.value || ''
          })
        }
      }, 100)
    }
